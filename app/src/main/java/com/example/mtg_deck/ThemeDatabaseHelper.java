...
public class ThemeDatabaseHelper extends SQLiteOpenHelper {
 ...
 private static void insertEnters2(SQLiteDatabase db, String tab, String theme, String text, int com) {
  ContentValues textEnters = new ContentValues();
  textEnters.put("TAB", tab);
  textEnters.put("THEME", theme);
  textEnters.put("TEXT", text);
  textEnters.put("COM", com);
  db.insert("TEXT_THEME", null, textEnters);
    ...
    public void onCreate(SQLiteDatabase db) {
     ...
     insertEnters2(db, "", "Типы", "Примитивные типы Java не являются объектами. К ним относятся:", 0);
     insertEnters2(db, " ", "Типы", " boolean - булев тип, может иметь значения true или false", 0);
     insertEnters2(db, " ", "Типы", "byte - 8-разрядное целое число", 0);
     insertEnters2(db, " ", "Типы", " short - 16-разрядное целое число", 0);
     insertEnters2(db, " ", "Типы", " int - 32-разрядное целое число", 0);
     insertEnters2(db, " ", "Типы", " long - 64-разрядное целое число", 0);
     insertEnters2(db, " ", "Типы", " char - 16-разрядное беззнаковое целое, представляющее собой символ UTF-16 (буквы и цифры)", 0);
     insertEnters2(db, " ", "Типы", "float - 32-разрядное число в формате IEEE 754 с плавающей точкой", 0);
     insertEnters2(db, " ", "Типы", " double - 64-разрядное число в формате IEEE 754 с плавающей точкой", 0);
     insertEnters2(db, " ", "Типы", " Примитивный в данном случае не оскорбление, а просто даёт понять, что речь идёт о простом типе, который не умеет прыгать, спать или мяукать. Да что он вообще умеет? Ой, всё."
             ...
             + " Объявить переменную типа byte можно следующим образом: ", 0);

     insertEnters2(db, " ", "Типы", "", 0);
     insertEnters2(db, " ", "Типы", " byte c, a, t; // объявили сразу три переменные ", 0);
     insertEnters2(db, " ", "Типы", " ", 0);
     insertEnters2(db, " ", "Типы", " В арифметических выражениях с переменными типа byte вычисления выполняются как с типом int, т.е. с помощью 32-битовой арифметики, а полученный результат будет 32-битовым. Смотри пример с short." +
                     ...
                     + " Класс Byte является оболочкой для данного типа. Без необходимости не используйте в Android класс Byte. "
                     + " Слово «байт» (byte) возникло в компании IBM примерно в 1956 году. Оно произошло от слова bite («кусок»), но его было решено писать через букву y, чтобы не путать со словом «bit» («бит»). В течение некоторого времени слово «байт» обозначало просто число битов в конкретном потоке данных. Однако в середине 1960-х, в связи с разработкой семейства компьютеров System/360 в компании IBM, это слово стало обозначать группу из восьми бит. "
             + " Любопытно, что bite имеет также значение 'укус' (сущ.) или 'укусить' (глагол). Таким образом это наш родной 'Кусь!'", 0);

     insertEnters2(db," ", "Типы", "\n\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tВопросы\t\t", 1);
     insertEnters2(db, "", "Побитовые операторы", "Существует несколько побитовых операторов, применимых к целочисленными типам long, int, short, char, byte.", 0);
     insertEnters2(db, "", "Побитовые операторы", "~\tПобитовый унарный оператор NOT", 0);
     insertEnters2(db, "", "Побитовые операторы", "&\tПобитовый AND", 0);
     insertEnters2(db, "", "Побитовые операторы", "&=\tПобитовый AND с присваиванием", 0);
     insertEnters2(db, "", "Побитовые операторы", "|\tПобитовый OR", 0);
     insertEnters2(db, "", "Побитовые операторы", "|=\tПобитовый OR с присваиванием", 0);
     insertEnters2(db, "", "Побитовые операторы", "^\tПобитовый исключающее OR", 0);
     insertEnters2(db, "", "Побитовые операторы", "^=\tПобитовый исключающее OR с присваиванием", 0);
     insertEnters2(db, "", "Побитовые операторы", ">>\tСдвиг вправо", 0);
     insertEnters2(db, "", "Побитовые операторы", ">>=\tСдвиг вправо с присваиванием", 0);
     insertEnters2(db, "", "Побитовые операторы", ">>>\tСдвиг вправо с заполнением нулями", 0);
     insertEnters2(db, "", "Побитовые операторы", "<<\tСдвиг влево", 0);
     insertEnters2(db, "", "Побитовые операторы", "<<=\tСдвиг влево с присваиванием", 0);
     insertEnters2(db, "", "Побитовые операторы", ">>>=\tСдвиг вправо с заполнением нулями с присваиванием", 0);
     insertEnters2(db, "", "Побитовые операторы", "Все целочисленные типы представляются двоичными числами различной длины. Например, значение типа byte, равное 42, в двоичном представлении имеет вид 00101010, в котором каждая позиция представляет степень числа два."
             ...
             + "\n"
             + "\nДвоичное дополнение используется в большинстве компьютерных языков. Опустим теорию, нужно только помнить, что старший бит определяет знак целого числа.", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "Побитовые логические операторы", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "Побитовые логические операторы - это &, |, ^, ~. Побитовые операторы применяются к каждому отдельному биту каждого операнда.", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "Результаты выполнения побитовых логических операторов", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     ...
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "Побитовое OR (|)", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "Результирующий бит, полученный в результате выполнения оператора OR, |, равен 1, если соответствующий бит в любом из операндов равен 1.", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", " 00101010  42\n" +
             ...
             "  00101111  47 ", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "Побитовое AND (&)", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "Значение бита, полученное в результате выполнения побитового оператора AND, &, равно 1, если соответствующие биты в операндах также равны 1. Во всех остальных случаях значение результирующего бита равно 0.", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "  00101010  42\n" +
             "& 00001111  15\n" +
             "--------------\n" +
             "  00001010  10",0);
     insertEnters2(db, "", "Побитовые операторы", "",0);
     insertEnters2(db, "", "Побитовые операторы", "Побитовое XOR (^)",0);
     insertEnters2(db, "", "Побитовые операторы", "",0);
     insertEnters2(db, "", "Побитовые операторы", "Результирующий бит, полученный в результате выполнения оператора XOR, ^, равен 1, если соответствующий бит только в одном из операндов равен 1. Во всех других случаях результирующий бит равен 0.", 0);
     insertEnters2(db, "", "Побитовые операторы", "",0);
     insertEnters2(db, "", "Побитовые операторы", " 00101010  42\n" +
             "^ 00001111  15\n" +
             "--------------\n" +
             "  00100101  37  ",0);
     insertEnters2(db, "", "Побитовые операторы", "Данный оператор обладает полезной особенностью, часто используемый в программировании. Например, можно обменять значения переменных без использования дополнительной переменной",0 );
     ...
     "\tbyte[] important = encode(message, catkey);\n" +
             ...
             "}", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "Побитовое NOT (~)", 0);
     insertEnters2(db, "", "Побитовые операторы", "Унарный оператор NOT (Не), ~, называемый также побитовым дополнением, инвертирует все биты операнда. Например, число 42 в битовом представлении имеет вид:",0);
     insertEnters2(db, "", "Побитовые операторы", "00101010", 0);
     insertEnters2(db, "", "Побитовые операторы", "В результате применения оператора NOT преобразуется в значение:", 0);
     insertEnters2(db, "", "Побитовые операторы", "11010101", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "Сдвиг влево", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "Оператор сдвига влево, <<, смещает все биты влево на указанное количество позиций:", 0);
     insertEnters2(db, "", "Побитовые операторы", "значение << количество", 0);
     ...
     insertEnters2(db, "", "Побитовые операторы", "\n" +
             ...
             "mInfoTextView.append(\"y равно: \" + y);", 0);
     insertEnters2(db, "", "Побитовые операторы", "Результат будет следующим:", 0);
     ...
     insertEnters2(db, "", "Побитовые операторы", "Поскольку для выполнения вычислений тип переменной повышается до int, сдвиг влево на две позиции значение 64 (0100 0000) приводит к значению 256 (1 0000 0000). Однако, переменная y содержит значение не 256, а 0, поскольку после сдвига крайний единичный бит оказывается сдвинутым за пределы допустимого диапазона.\n" +
             "\n" +
             "Приёмом сдвига влево часто пользуются в программах, где происходит много сложных вычислений. По сути, это замена операции умножения на 2, которая в силу особенностей процессора гораздо эффективнее. При этом следует соблюдать осторожность, так как при сдвиге единичного бита в старшую позицию (бит 31 или 63) значение становится отрицательным.", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "Сдвиг вправо", 0);
     insertEnters2(db, "", "Побитовые операторы", "Оператор сдвига вправо, >>, смещает все биты значения вправо на указанное количество позиций:", 0);
     insertEnters2(db, "", "Побитовые операторы", "значение >> количество", 0);
     insertEnters2(db, "", "Побитовые операторы", "Вы указываете количество позиций, на которое нужно сдвинуть вправо биты в значении. Следующий код выполняет сдвиг право на две позиции в значении 32, в результате чего значение переменной станет равным 8.", 0);
     insertEnters2(db, "", "Побитовые операторы", "int a = 32;\n" +
             "a = a >> 2 // стало равным 8", 0);
     insertEnters2(db, "", "Побитовые операторы", "Крайние биты при сдвиге просто теряются. Например, значение 35 при сдвиге вправо на две позиции также приводит к значению 8, так как теряются два младщих бита.", 0);
     ...
     "При выполнении сдвига вправо старшие (расположенные в крайних левых позициях) биты, освобожденные в результате сдвига, заполняются предыдущим содержимым старшего бита. Этот эффект называется дополнительных знаковым разрядом и служит для сохранения знака отрицательных чисел при их сдвиге.\n" +
             ...
             "\n" +
             "Иногда при выполнении сдвига вправо появление дополнительных знаковых разрядов нежелательно. В этом случае используется маскировка за счёт объединения значения со значением 0x0f оператором AND, что приводит к отбрасыванию любых битов дополнительных знаковых разрядов.", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "Сдвиг вправо без учёта знака", 0);
     insertEnters2(db, "", "Побитовые операторы", "Как было показано выше, при каждом выполнении оператор >> автоматически заполняет старший бит его предыдущим содержимым. В результате знак значения сохраняется. Однако иногда это нежелательно. Например, при работе со значениями пикселей и графическими изображениями. Как правило, в этих случаях требуется сдвиг нуля в позицию старшего бита независимо от его первоначального значения. Такое действие называют сдвигом вправо без учета знака, когда всегда вставляется нуль в позицию старшего бита.\n" +
             "\n" +
             "Допустим, мы хотим сдвинуть вправо на 24 бит значение -1 (в двоичном представлении 11111111 11111111 11111111 11111111):", 0);
     insertEnters2(db, "", "Побитовые операторы", "    11111111 11111111 11111111 11111111\n" +
             ">>> 24\n" +
             "---------------------------------------\n" +
             "    00000000 00000000 00000000 11111111 255 в двоичном виде типа int", 0);
     insertEnters2(db, "", "Побитовые операторы", "Оператор >>> не столь полезен, поскольку имеет смысл только для 32- и 64-разрядных значений.", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "Побитовые составные операторы с присваиванием", 0);
     insertEnters2(db, "", "Побитовые операторы", "", 0);
     insertEnters2(db, "", "Побитовые операторы", "Двоичные побитовые операторы имеют составную форму, которая объединяет побитовый оператор с оператором присваивания.", 0);
     insertEnters2(db, "", "Побитовые операторы", "\n" +
             "a = a >> 4;\n" +
             "a >>= 4;\n" +
             "\n" +
     ...
     insertEnters2(db, "", "Операторы сравнения", "Наиболее часто операции сравнения используют в выражениях, которые управляют оператором if и операторами цикла.\n" +
             ...
             "\n" +
             "Корректный вариант примера:",0);
     insertEnters2(db, "", "Операторы сравнения", "\n" +
             "int x = 5;\n" +
             "int y = 7;\n" +
             "boolean z = a < b; // результат сохраняется в переменной типа boolean", 0);
     insertEnters2(db, "", "Операторы сравнения", "Примеры для оператора if для Java (В C++ используется другой синтаксис!):", 0);
     insertEnters2(db, "", "Операторы сравнения", "\n" +
             "int cat_age;\n" +
             "if(cat_age == 4) // нельзя if(cat_age) - нет сравнения\n" +
             "if(cat_age != 9) // нельзя if(!cat_age) - нет сравнения", 0);
     insertEnters2(db, "", "Операторы сравнения", "Помните, что в Java значения true и false не являются числовыми значениями, как в C++, поэтому, чтобы сравнить значение с другим значением, необходимо явно использовать операторы сравнения.", 0);


     insertEnters2(db, "", "Логические операторы", "Логические операторы", 0);
     insertEnters2(db, "", "Логические операторы", "", 0);
     insertEnters2(db, "", "Логические операторы", "Логические операторы работают только с операндами типа boolean. Все логические операторы с двумя операндами объединяют два логических значения, образуя результирующее логическое значения. Не путайте с побитовыми логическими операторами.",0);
     insertEnters2(db, "", "Логические операторы", "", 0);
     insertEnters2(db, "", "Логические операторы", "Таблица логических операторов в Java", 0);
     insertEnters2(db, "", "Логические операторы", "Оператор                  Описание\n" +
             ...
             "==                 Равно\n" +
             "!=                 Не равно\n" +
             "?:                 Тернарный (троичный) условный оператор", 0);
     insertEnters2(db, "", "Логические операторы", "Логические операторы &, |, ^ действуют применительно к значениям типа boolean точно так же, как и по отношению к битам целочисленных значений. Логический оператор ! инвертирует (меняет на противоположный) булево состояние: !true == false и !false == true.", 0);
     insertEnters2(db, "", "Логические операторы", "", 0);
     insertEnters2(db, "", "Логические операторы", "Таблица. Результаты выполнения логических операторов",0);
     ...
     insertEnters2(db, "", "Логические операторы", "", 0);
     insertEnters2(db, "", "Логические операторы", "Сокращённые логические операторы", 0);
     insertEnters2(db, "", "Логические операторы", "", 0);
     insertEnters2(db, "", "Логические операторы", "Кроме стандартных операторов AND (&) и OR (|) существуют сокращённые операторы && и .\n" +
             ...
             "\n" +
             "Рассмотрим следующий пример. Допустим, мы ввели правило - кормить или не кормить кота в зависимости от числа пойманных мышек в неделю. Причём число мышек зависит от веса кота. Чем больше кот, тем больше он должен поймать мышей.",0);
     ...
     insertEnters2(db, "", "Логические операторы", "Если запустить программу, то пример будет работать без проблем - пять мышей в неделю вполне достаточно, чтобы побаловать кота вкусным завтраком. Если он поймает четырёх мышей, то начнутся проблемы с питанием кота, но не с программой - она будет работать, просто не будет выводить сообщение о разрешении покормить дармоеда.\n" +
             ...
             "\n" +
             "Перепишем условие следующим образом (добавим всего лишь один символ):", 0);
     insertEnters2(db, "", "Логические операторы", "\n" +
             "if (mouse != 0 && weight / mouse < 1000) {\n" +
             "\tmInfoTextView.setText(\"Можно кормить кота\");\n" +
             "}",0 );
     insertEnters2(db, "", "Логические операторы", "Теперь программа работает без краха. Как только Java увидела, что первое выражение возвращает false, то второе выражение с делением просто игнорируется.\n" +
             "\n" +
             "Сокращённые варианты операторов AND и OR принято использовать в тех ситуациях, когда требуются операторы булевой логики, а их односимвольные родственники используются для побитовых операций.", 0);
     insertEnters2(db, "", "Логические операторы", "", 0);
     insertEnters2(db, "", "Логические операторы", "Тернарный оператор", 0);
     insertEnters2(db, "", "Логические операторы", "", 0);
     insertEnters2(db, "", "Логические операторы", "В языке Java есть также специальный тернарный условный оператор, которым можно заменить определённые типы операторов if-then-else - это оператор ?:\n" +
             ...
             "\n", 0);
     insertEnters2(db, "", "Логические операторы", "логическоеУсловие ? выражение1 : выражение2", 0);
     insertEnters2(db, "", "Логические операторы", "Если логическоеУсловие равно true, то вычисляется выражение1 и его результат становится результатом выполнения всего оператора. Если же логическоеУсловие равно false, то вычисляется выражение2, и его значение становится результатом работы оператора. Оба операнда выражение1 и выражение2 должны возвращать значение одинакового (или совместимого) типа.\n" +
             "\n" +
             "Рассмотрим пример, в котором переменной absval присваивается абсолютное значение переменной val.",0);
     ...
     "if(lowNum < highNum)  // если первое число меньше второго\n" +
             ...
             "\n" +
             "Нельзя сказать, что такой код слишком читаем, поэтому можно не использовать в своей практике.",0);
